...
var instrucciones = ['mueve las fichas usando las flechas', 'el objetivo del juego es completar la imagen','triva: pikachu es un mal pokémon en todos los títulos, pero sigue siendo el más popular'];
...
function ultimosMovimientos(direccion){
...
function chequearSiGano() {
  for(let i = 0; i < grilla.length; i++){
    for(let j = 0; j < grilla[i].length; j++){
      if(grilla[i][j] !== winningGrilla[i][j]){
        return false;
      }
...
function intercambiarPosicionesGrilla(filaPos1, columnaPos1, filaPos2, columnaPos2) {
  var grillaTemp = grilla[filaPos1][columnaPos1];
  grilla[filaPos1][columnaPos1] = grilla[filaPos2][columnaPos2];
  grilla[filaPos2][columnaPos2] = grillaTemp;
}
